export class shipmentRouting {
    ...
    searchDragnDropRecordByRefNo() {
        cy.get('@forwarderReferenceNumber').then(fwdRefNo => {
            ...
            cy.get('tr.shipment-list-item').contains('td', new RegExp(fwdRefNo, 'i')).parent('tr').find('input[type="checkbox"]').check({ force: true });
            ...
            cy.get('tr.shipment-list-item.is-selected').first().then($source => {
                cy.get('div.route-shipments').first().then($target => {
                    ...
                    //.should('match', new RegExp(fwdRefNo));
        ...
        cy.get('@forwarderReferenceNumber').then(fwdRefNo => {
            cy.get(this.weblocators.firstRoutingVehicleCard).first().then($source => {
                cy.get('div.route-shipments').first().then($target => {
                    ...
                    cy.wait(8000);
                    cy.get(this.weblocators.searchInputByReferenceNumber).scrollIntoView().should('be.visible');
                    cy.get('tr.shipment-list-item').contains('td', new RegExp(fwdRefNo, 'i'));
                });
            });
        });
    ...
    saveAndUnDispatchRoute(driver1) {
        cy.get(this.weblocators.driverSelectDropdown).select(driver1);
        cy.get('@forwarderReferenceNumber').then(fwdRefNo => {
            cy.get(this.weblocators.firstRoutingVehicleCard).first().then($source => {
                cy.get('div.route-shipments').first().then($target => {
                    ...
                    cy.get(this.weblocators.primaryButton).contains('Dispatch Route').click();
                    cy.wait(8000);
                    cy.get('button.button.button--delete').contains('Undispatch Driver').click({ force: true });
                    cy.get(this.weblocators.confirmRouteCompletionButton).contains(' Confirm ').click();
                    cy.wait(8000);
                    cy.get('button.icon-blue').eq(1).click({ force: true });
                    cy.wait(5000);
                    cy.get('button.icon-blue').eq(0).click({ force: true });
                    cy.wait(5000);
                    cy.get('div.routing-shipment-card').should('not.exist');
                });
            });
        });
    }
